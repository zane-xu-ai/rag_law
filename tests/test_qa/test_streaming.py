...
class _FakeChoice:
    def __init__(self, content: str | None = None, *, reasoning_content: str | None = None) -> None:
        self.delta = _FakeDelta(content, reasoning_content=reasoning_content)
...
def _fake_stream_with_reasoning() -> Any:
    yield _FakeChunk(reasoning_content="考")
    yield _FakeChunk("答")
    yield _FakeChunk("案")
...
def test_stream_qa_events_reasoning_delta_and_done_reasoning_text(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """流式含 reasoning_content 时产出 reasoning_delta，done 带 reasoning_text。"""
    _base_env(monkeypatch)
    get_settings.cache_clear()
    settings = get_settings()
    ...
    class FakeCompletions:
        ...
        def create(**kw: Any) -> Any:
            assert kw.get("stream") is True
            return _fake_stream_with_reasoning()
    ...
    content_deltas = [e for e in events if e["type"] == "delta"]
    assert "".join(e["content"] for e in content_deltas) == "答案"
    done = [e for e in events if e["type"] == "done"][0]
    assert done["reasoning_text"] == "考"
    assert done["text"] == "答案"
