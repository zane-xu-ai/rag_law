...
| **路线状态** | **方案 C（嵌入断点）、方案 D（分段模型）暂不推进**，后续再评估排期；当前入库主线见 [v1.0.4-ingest-plan.md §12](v1.0.4-ingest-plan.md)（OSS → 本地 → 切分 → 向量 → ES）。 |
...
> **状态：暂停。** 本节保留设计备忘；实施与评测排期后移，不与当前 OSS 入库主线并行。
...
**验收**：与方案 A/B 共用同一套文档（如宪法 golden）做块数、条完整性、抽样问答对比。
...
| §2 方案 C / §3 方案 D | **暂停**（仅设计备忘，未编码、当前不排期） |
