...
1. [ ] FastAPI lifespan：`app.state` 注入 embedder、Elasticsearch、OpenAI 单例。
2. [ ] `stream_qa_events` 改为使用注入的 embedder/客户端，移除每请求 `build_embedder`。
3. [ ] 可选：startup 对 embedder 执行一次 dummy `embed_query`。
4. [ ] 更新 `tests/test_qa/test_streaming.py` / `test_webui_app.py`：传入 mock 单例或保持可注入。
5. [ ] README 或 v1.0.7 文档补充 worker 与显存说明。
