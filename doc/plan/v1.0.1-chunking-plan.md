...
- 代码：[src/chunking/boundary.py](../../src/chunking/boundary.py)；`iter_chunks_for_text(..., boundary_aware=True)` 启用；**默认 `False`**，与纯滑窗兼容。
- 预览 Web：请求体字段 **`boundary_aware`**（或 multipart 表单 `boundary_aware=true`）。
...
| 版本 | 日期 | 说明 |
| --- | --- | --- |
| v1.0.1 | 2026-04 | 切分实现计划（语料统计、CHUNK 建议、`src/chunking` 布局与测试）；后补句边界对齐可选策略（[句边界对齐切分.md](../chunk/句边界对齐切分.md)、`boundary_aware`） |
