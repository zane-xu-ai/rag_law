...
| --- | --- |
| 文档版本 | v1.0.5 |
| 状态 | 已实施（[`src/qa/`](../../src/qa/)、[`scripts/rag_qa.py`](../../scripts/rag_qa.py)） |
...
**后续 Web 界面**：已实现，见 [v1.0.7-qa-webui-plan.md](v1.0.7-qa-webui-plan.md)（[`src/qa/webui/`](../../src/qa/webui/)）。
...
1. [x] 新建 `src/qa/`：`prompts` 或单模块内常量 + `build_messages`。
2. [x] 实现 `answer_question`（或 `run_qa_pipeline`）：embed → `EsChunkStore.search_knn` → OpenAI → 返回 `content`。
3. [x] 新增 `scripts/rag_qa.py`（CLI、`--dry-run`、`--k`）。
4. [x] 单元测试 + README 小节；[v1.0.0-rag-law-mvp-plan.md](v1.0.0-rag-law-mvp-plan.md) §9「问答」勾选（实施完成后）。
...
| v1.0.5 修订 | 2026-04-05 | 落地 `src/qa/messages.py`、`pipeline.py`、`scripts/rag_qa.py`、单测与 `QA_MAX_CONTEXT_CHARS` |
