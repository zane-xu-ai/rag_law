...
- [ ] **向量**：加载本地 BGE-M3，封装 `embed_documents` / `embed_query`，与 ES `similarity` 约定一致。
- [ ] **ES**：创建索引 mapping；bulk 写入；实现 `search(query_vector, k)`。
- [ ] **入库**：端到端跑通（MVP 可每次全量重建索引）。
- [ ] **问答**：拼接 system/user 提示（引用依据、无法律依据时声明）；调用兼容接口返回答案。
