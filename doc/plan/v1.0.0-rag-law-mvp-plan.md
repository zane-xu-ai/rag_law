...
- **配置层**：读取 `.env`；提供 `.env.example`；统一 chunk / ES / 模型参数命名。
- **切分**：遍历 `data/*.md`，UTF-8 读取，按 `CHUNK_SIZE` / `CHUNK_OVERLAP` 生成块并附元数据。
- **向量**：加载本地 BGE-M3，封装 `embed_documents` / `embed_query`（见 `[v1.0.2-bge-m3-embedding-plan.md](v1.0.2-bge-m3-embedding-plan.md)`）；稠密向量 **L2 归一化** 后与 ES `dense_vector` + `**cosine`** 一致。
- **ES**：创建索引 mapping；bulk 写入；`search(query_vector, k)`（见 `[v1.0.3-es-store-plan.md](v1.0.3-es-store-plan.md)`、`[src/es_store](../../src/es_store/)`）。
- **入库**：端到端跑通（MVP 可每次全量重建索引）。实施方案与命令见 `[v1.0.4-ingest-plan.md](v1.0.4-ingest-plan.md)`（`[scripts/rag_ingest.py](../../scripts/rag_ingest.py)`）。
- **问答**：拼接 system/user 提示（引用依据、无法律依据时声明）；调用兼容接口返回答案。实施方案见 `[v1.0.5-qa-plan.md](v1.0.5-qa-plan.md)`；CLI：`[scripts/rag_qa.py](../../scripts/rag_qa.py)`。开发前可先验证 `MODEL_`* 连通性：`[scripts/llm_smoke_test.py](../../scripts/llm_smoke_test.py)`（`uv sync --extra llm`，见 `[README.md](../../README.md)`）。
- **验收**：1～2 个法律问题，检查检索 chunk 与答案是否贴合条文。操作步骤与检查清单见 `[v1.0.6-mvp-acceptance-plan.md](v1.0.6-mvp-acceptance-plan.md)`。
...
| 5   | 问答                 | query 向量化 + 检索 + prompt + LLM（见 `[v1.0.5-qa-plan.md](v1.0.5-qa-plan.md)`）            |
| 6   | 验收                 | 人工 1～2 问、核对检索与答案（见 `[v1.0.6-mvp-acceptance-plan.md](v1.0.6-mvp-acceptance-plan.md)`） |
...
| v1.0.0 修订 | 2026-04    | 新增问答阶段专项计划 `[v1.0.5-qa-plan.md](v1.0.5-qa-plan.md)`，§9「问答」任务链入                                                                |
| v1.0.0 修订 | 2026-04-05 | §9「问答」勾选完成：`[v1.0.5-qa-plan.md](v1.0.5-qa-plan.md)`、`[src/qa/](../../src/qa/)`、`[scripts/rag_qa.py](../../scripts/rag_qa.py)` |
| v1.0.0 修订 | 2026-04-05 | §9「验收」勾选完成：`[v1.0.6-mvp-acceptance-plan.md](v1.0.6-mvp-acceptance-plan.md)`                                                   |
