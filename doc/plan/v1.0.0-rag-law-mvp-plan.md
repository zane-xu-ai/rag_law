...
- [x] **问答**：拼接 system/user 提示（引用依据、无法律依据时声明）；调用兼容接口返回答案。实施方案见 [`v1.0.5-qa-plan.md`](v1.0.5-qa-plan.md)；CLI：[`scripts/rag_qa.py`](../../scripts/rag_qa.py)。开发前可先验证 `MODEL_*` 连通性：[`scripts/llm_smoke_test.py`](../../scripts/llm_smoke_test.py)（`uv sync --extra llm`，见 [`README.md`](../../README.md)）。
- [x] **验收**：1～2 个法律问题，检查检索 chunk 与答案是否贴合条文。操作步骤与检查清单见 [`v1.0.6-mvp-acceptance-plan.md`](v1.0.6-mvp-acceptance-plan.md)。
