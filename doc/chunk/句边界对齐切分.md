...
8. **异常**：若调整后仍 `start >= end`，回退到该段初值 `(s0, e0)` 并再次施加块链约束；若仍无效，将 `end` 扩展为至少 `start + 1` 或跳过（见实现）。
9. **实现注意**：若某窗在修正后 `start >= len(text)`，应**跳过该窗并继续处理后续滑窗**（`continue`），不得提前终止整个迭代，否则长文本会表现为块数锐减或尾部丢失。
