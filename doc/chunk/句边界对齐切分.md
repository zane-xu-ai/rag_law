...
| 相关文档 | [Chunking 切分效果人工测试v01.md](Chunking%20切分效果人工测试v01.md)、[句边界块链与二次对齐方案.md](句边界块链与二次对齐方案.md)（扩展扫描、夹紧后二次句首对齐、重叠冲突策略） |
...
2. **每段内**：先对句首、句尾做 **`adjust_start_extended` / `adjust_end_extended`**（在局部 ±`max_probe` 无合适断点时，以 **`max_boundary_scan`** 半径再做强→弱，默认有效半径为 `max(max_probe, min(chunk_size, 800))`）；对起点施加重叠夹紧后，再对夹紧结果跑一次 **`adjust_start_extended`**，并由 **`CHUNK_BOUNDARY_PRIORITY_OVERLAP`** 等在「重叠区间」与「句首合法」之间协调（见 [句边界块链与二次对齐方案.md](句边界块链与二次对齐方案.md)）。
...
5. **为何对齐后块长几乎总是 ≤ chunk_size、很少「大于 1500」**（属**预期**，非滑窗 bug）：
   - `chunk_size` 是**滑窗初值**的片段长度（`e0 - s0`），**不是**「每块必须写满」或「对齐后长度下限」。
   - **句尾** `adjust_end` 会把右开终点**挪到**最近的强/弱句界（在 ±`max_probe` 内），多数情况下相对 `e0` **前移** → 块**变短**。
   - **句首** `adjust_start` 常把起点**挪到**句界之后，相对 `s0` **后移** → 同样**变短**。
   - **重叠夹紧**要求 `start` 落在 `[prev_end - overlap_ceiling, prev_end - overlap_floor]`，常需把对齐后的起点**再右移** → 本块**更短**。
   - **理论上界**：若句界恰好使 `start` 早于 `s0` 且 `end` 晚于 `e0`（两侧同时「向外」），单块最长约为 **`chunk_size + 2 * max_probe`**（如 1500+60）；法律正文中句界多在窗口**内侧**拉扯，**缩短**远多于**拉长**，故统计上**几乎看不到**大于 `chunk_size` 的块。
   - **末块**短于 `chunk_size` 是**自然**的（原文剩余不足一窗）。
6. **「首尾不完整」多**：若按 `BOUNDARY_CHARS`（`。！？；` 与换行）判「句界完整」，则**逗号、顿号、英文句号**等都不算句界，中文长句里**大量**块会以逗号结尾或从逗号后开头，统计上会显示很多「不完整」——这是**规则定义**所致，可用 `scripts/analyze_boundary_chunk_integrity.py` 对照明细原因。
7. **异常**：若调整后仍 `start >= end`，回退到该段初值 `(s0, e0)` 并再次施加块链约束；若仍无效，将 `end` 扩展为至少 `start + 1` 或跳过（见实现）。
8. **实现注意**：若某窗在修正后 `start >= len(text)`，应**跳过该窗并继续处理后续滑窗**（`continue`），不得提前终止整个迭代，否则长文本会表现为块数锐减或尾部丢失。
...
| 项目 | 说明 |
| --- | --- |
| 模块 | [boundary.py](../../src/chunking/boundary.py)：`adjust_start` / `adjust_start_extended`、`adjust_end` / `adjust_end_extended`、`iter_text_slices_boundary_aware`、`iter_boundary_aware_diag_rows` |
