...
3. **块间约束（重叠下界 `overlap_floor`）**：滑窗目标重叠为 `chunk_overlap`（步长 `chunk_size - chunk_overlap`）。句界对齐后，与上一块实际重叠允许 **低于** 该目标，但不得低于配置 **`CHUNK_OVERLAP_MIN`（代码中为 `overlap_floor`，默认未设时等于 `chunk_overlap`）**。记上一块右开终点为 `prev_end`，则本块起点需满足 `prev_end - start >= overlap_floor`，即 **`start <= prev_end - overlap_floor`**，在句界对齐得到 `s_adj` 后取 **`min(s_adj, prev_end - overlap_floor)`**。若误用 **`max(...)`** 作下界，会把起点强行右推，导致块首落在句中（如「的统一领导下…」）。当 `overlap_floor < chunk_overlap` 时，句界探测「吃掉」的部分重叠可落在 `[overlap_floor, chunk_overlap]` 之间浮动，而不必强行左移起点以凑满目标重叠。
4. **块长与重叠**：对齐后实际块长可能偏离 `chunk_size`；实际重叠可在 **`overlap_floor` 与约 `chunk_overlap` 之间**浮动（受句界与 `max_probe` 影响）。
